...
def train(epoch):
    ...
    for i, (images, labels) in enumerate(data_train_loader):
        ...
        loss.backward()
        optimizer.step()
    torch.save(net.state_dict(),'./checkpoints/bestModel.pth')
...
def inference():
    #单独写的推理模块
    net = LeNet5()
    # summary(net,input_size=(6,1,5,5))
    ...
    # converter = ModelConverter(qconfig,mapping=mapping)
    # converter(net)
    # toggle(net,quantize=True, observer=True,calc_qparam=True)

    net.eval()
    #print(net,'quant_model')
    #x1= time.time()
    ...
    #print (x2-x1)
    avg_loss /= len(data_test)
    print('The inference results as follows----------')
    print('Test Avg. Loss: %f, Accuracy: %f' % (avg_loss.detach().cpu().item(), float(total_correct) / len(data_test)))    
