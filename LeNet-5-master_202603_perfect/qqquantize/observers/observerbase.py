...
class ObserverBase(ABC, nn.Module):
    ...
    `calculate_qparam` function that computes the quantization parameters given
    ...
    @abstractmethod
    def calculate_qparam(self, **kwargs):
        pass

    with_args = classmethod(_with_args)

def calculate_qparam_symmetric(min_val, max_val, qmin, qmax):
    """quant_x = int(float_x / scale)
    scale is the unit size
    zero_point is always 0"""
    if min_val == 0 or max_val == 0:
        raise ValueError(
            "must run observer before calling calculate_qparam.\
